...
public class BezierSurfaceWing : MonoBehaviour
{
    ...
    void InitializeControlPoints()
    {
        ...
        float chordLength = controlPointSpacing * (controlPointsU - 1);
        float maxCamber   = controlPointSpacing * 0.6f;
        float maxThickness = controlPointSpacing * 0.8f;

        BoxCollider box = GetComponent<BoxCollider>();
        box.size = new Vector3(chordLength, maxThickness/2, chordLength);
        box.center = new Vector3(chordLength/3, 0, 0);


        for (int u = 0; u < controlPointsU; u++)
        {
            // Normalized chord position [0,1]
            float t = (float)u / (controlPointsU - 1);

            // Close the surface: leading edge == trailing edge
            float chordX = ((u == controlPointsU - 1)||(u==0)) ? maxCamber*3 : Mathf.Cos(t * 2* Mathf.PI) * maxCamber*2;

            // Simple aerofoil camber curve
            float camberY = Mathf.Sin(t * 2* Mathf.PI) * maxCamber;

            for (int v = 0; v < controlPointsV; v++)
            {
                // Normalized thickness direction [-1,1]
                float s = (controlPointsV == 1)
                    ? 0f
                    : (v / (float)(controlPointsV - 1)) * 2f - 1f;

                float thicknessZ = s * maxThickness;
                ...
                cp.transform.SetParent(transform, true); // keep world position
                cp.name = $"CP_{u}_{v}";
